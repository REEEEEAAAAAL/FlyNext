...
async function main() {
	...
	await prisma.city.createMany({
		data: cities.map((city: { city: string; country: string }) => ({
			name: city.city,
	...
	const cityNameToId = new Map(
		cityRecords.map((city: { name: string; id: number }) => [city.name, city.id])
	);
	...
	await prisma.airport.createMany({
		data: airports.map((airport: { id: number; code: string; name: string; city: string; country: string }) => {
